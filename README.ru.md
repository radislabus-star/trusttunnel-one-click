...
## Подключить Linux-компьютер к готовому VPN

Попросите администратора прислать личную ссылку `tt://`, затем выполните на
Ubuntu или Debian:
...
Установщик скрыто запросит ссылку, проверит GPG-подписи официального
TrustTunnelClient, создаст закрытую конфигурацию и systemd-сервис с
автозапуском. Ссылка не сохраняется в истории shell. Внутри SSH-сеанса VPN
автоматически не запускается и не включается в автозагрузку, чтобы не оборвать
удалённый доступ.
...
## Установить собственный VPN-сервер
